...
const draftEmailResponse = async (id, requestJSON) => {
  try {
    ...
    let claude = new ClaudeModel();
    const stream = await claude.getStreamedResponse(
      data.systemPrompt,
      history,
    ...
    if (!stream) {
      throw new Error("Failed to get response stream from Claude model");
    }
    ...
    let responseParams = {
      ConnectionId: id,
      Data: `<!ERROR!>: Failed to generate email: ${e.message}`
    }
    let command = new PostToConnectionCommand(responseParams);
    await wsConnectionClient.send(command);
  }
