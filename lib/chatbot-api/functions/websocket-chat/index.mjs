          ...
          if (parsedChunk) {                      
            ...
            if (usingTool) {
              if (toolUse.name == "search") {
                toolUse.input.query = toolInput;
                message.content.push(toolUse);
                let searchResults = await knowledgeBase.send(new RetrieveCommand({
                ...
                if (searchResults.retrievalResults) {
                  for (const result of searchResults.retrievalResults) {
                    if (result.content && result.content.text) {
                      fullDocs.content = fullDocs.content.concat(result.content.text + "\n\n");
                      fullDocs.uris.push(result.location.s3Location.uri);
                      toolOutput.output.push(result.content.text);
                    }
                  }
                }
                
                message.content.push(toolOutput);
                history = claude.assembleHistory(lastFiveMessages, "Here are some relevant documents I found: \n\n".concat(fullDocs.content).concat("\n\nBased on these documents, let me answer your question about: ").concat(userMessage));
                usingTool = false;
                toolInput = "";
                break;
              }
              ...
              if (assemblingInput & !skipChunk) {
                ...
                // Clean the response chunk before sending
                const cleanedChunk = parsedChunk.toString()
                ...
                modelResponse = modelResponse.concat(cleanedChunk)
                let command = new PostToConnectionCommand(responseParams);
...
const draftEmailResponse = async (id, requestJSON) => {
  try {
    ...
    let emailPrompt = `Given this chat history, please draft an email that summarizes the resources that were discussed. Use the following format:
    Dear [Customer Name],
