...
const draftEmailResponse = async (id, requestJSON) => {
  try {
    ...
    let claude = new ClaudeModel();
    const stream = await claude.getStreamedResponse(emailPrompt, history);
    ...
    let responseParams = {
      ...
      Data: JSON.stringify({
        type: "email",
        content: await processBedrockStream(stream, claude)
      })
    };
    ...
    const command = new PostToConnectionCommand(errorParams);
    await wsConnectionClient.send(command);
  }
};
