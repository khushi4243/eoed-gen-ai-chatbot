...
const draftEmailResponse = async (id, requestJSON) => {
  try {
    const data = requestJSON.data;
    const historyObj = data.chatHistory;
    const prompt = data.systemPrompt;

    let claude = new ClaudeModel();

    let history = claude.assembleHistory(historyObj, "Please generate an email using the previous context to summarize the conversation about the resources offered by the EOED.")
